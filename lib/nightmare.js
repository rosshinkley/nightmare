...
function Nightmare(options) {
  ...
  this.proc = proc.spawn(electron_path, [runner, '--debug'].concat(JSON.stringify(electronArgs)), {
    stdio: [process.stdout, process.stdin, process.stderr, 'ipc']
  });
  ...
  this.proc.on('exit', function(code, signal){
    debug(`exited with ${signal} code ${code}`);
  });
