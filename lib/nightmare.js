...
function Nightmare(options) {
  ...
  var commandToSpawn = electron_path;
  var spawnArgs = [];

  if(process.env.CIRCLE_PROJECT_REPONAME || process.env.NIGHTMARE_USE_XVFB || options.virtualFramebufferArguments){
    //set up default arguments
    var xvfbargs = options.virtualFramebufferArguments = defaults(options.virtualFramebufferArguments || {}, {
      screenSize: '1280x1024x24+32',
      tmpDirectory: '/tmp',
      xvfbDirectory: `/tmp/xvfb.${process.pid}`,
      xvfbErrorLog: `/tmp/xvfb.${process.pid}/xvfb.log`
    });

    //ported from bb-xvfb
    //original shell script pulled from: https://gist.github.com/tullmann/2d8d38444c5e81a41b6d
    //original issue: https://github.com/angular/protractor/issues/2419#issuecomment-156527809

    //make the directory for the log
    mkdirp.sync(xvfbargs.xvfbDirectory);
    
    //make the electron arguments spawn-shell-friendly
    var stringifiedElectronArgs = JSON.stringify(electronArgs).replace(/"/g, '\\"');
    
    commandToSpawn='xvfb-run';
    
    //execute xvfb-run > waitforx > dbus > electron
    //dbus command to help avoid race conditions in chrome startup
    //see https://code.google.com/p/chromium/issues/detail?id=309093
    spawnArgs=[`--error-file=${xvfbargs.xvfbErrorLog}`,
      `--auto-servernum`,
      `--server-args="-fbdir ${xvfbargs.xvfbDirectory} -screen 0 ${xvfbargs.screenSize} -dpi 96 -ac"`,
      `dbus-launch`,
      `--exit-with-session`,
      `${electron_path}`,
      `${runner}`,
      `${stringifiedElectronArgs}`];

    this.proc = proc.spawn(commandToSpawn, spawnArgs, {
      stdio: [null, null, null, 'ipc'],
      shell: true
    });
  } else {
    spawnArgs.push(runner);
    spawnArgs.push(JSON.stringify(electronArgs));
    this.proc = proc.spawn(commandToSpawn, spawnArgs, {
      stdio: [null, null, null, 'ipc'],
    });
  }
...
Nightmare.prototype.end = function(done) {
  ...
  (queued('ending', function(done){
    debug('ending instance');
    child.once('ending', done);
    child.emit('ending');
  }).bind(this))();
