 ...
 * @param {Boolean} [options.offsetAbsolute=false],
...
exports.rightClick = function(selector, options, done) {
  ...
  debug('.rightClick() on ' + selector);
  this._evaluate(function (selector, options) {
    var element = document.querySelector(selector);
    var event = document.createEvent('MouseEvents');
    
    var x = 0, y = 0;
    if(options.offsetAbsolute){
       x = options.offsetLeft || 0;
       y = options.offsetTop || 0;
    } else {
        (function(element){
    ...
    event.initMouseEvent('contextmenu', true, true, element.ownerDocument.defaultView, options.clicks || 1, x, y, x, y, options.ctrlKey, options.altKey, options.shiftKey, options.metaKey, 2, null);
    element.dispatchEvent(event);
