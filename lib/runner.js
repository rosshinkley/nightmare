  ...
  parent.on('browser-initialize', function(options) {
    ...
    win.webContents.session.on('will-download', function(event, item, webContents){
      ...
      item.on('done', function(e, state){
        ...
        if(state == 'completed'){
          fs.renameSync(join(app.getPath('downloads'), item.getFilename()), downloadInfo.path);
        }
        parent.emit('download-done', downloadInfo); 
      });
      ...
      var handler = function(options){
        parent.removeListener('will-download', handler);
        if(options.filename == item.getFilename()){
          if(options.cancel){
            item.cancel();
          }
          else {
            if(options.path){
              ...
              downloadInfo.path = options.path;
            }
      ...
      parent.on('will-download', handler);
      parent.emit('will-download', downloadInfo);
    });
