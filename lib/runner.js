...
app.on('ready', function() {
  ...
  parent.on('type', function (value) {
    ...
    //add accelerators for special characters
    for (var specialCharacter in specialCharacters) {
      for (i in value) {
        var split = value[i].split(specialCharacter);
        if (split.length > 1) {
          var mod = [];
          for (var j = 0; j < split.length - 1; j++) {
            mod.push(split[j], specialCharacters[specialCharacter]);
          }
          mod.push(split[split.length - 1]);
          Array.prototype.splice.apply(value, [i, 1].concat(mod));
        }
      }
    };

    //split the array parts up into character parts _unless_ they are accelerator/modifiers
    var acceleratorCheck = new RegExp('^(?:'+accelerators.join('|')+')+(?:\\+(?:'+accelerators.join('|')+'))*');
    var toSend = [];
    for(i in value){
      if(!acceleratorCheck.test(value[i])){
        Array.prototype.push.apply(toSend, value[i].split(''));
      }else{
        toSend.push(value[i]);
      }
    }
