...
app.on('ready', function() {
  ...
  parent.on('browser-initialize', function(options) {
    ...
    //TODO: fill in download preferences?
    var downloadPreferences = defaults(options['download-preferences'], {
    });
    ...
    parent.emit('log', JSON.stringify(options,null,3));
    ...
    */
    ...
    win.webContents.session.on('will-download', function(event, item, webContents){
        parent.emit('log', 'will-download called');
        ...
        parent.emit('download-start', download);
        item.setSavePath(join(downloadPreferences.destination, item.getFilename()));

        item.on('done', function(e, state){
          parent.emit('log', 'download done, state: ' + state);
          parent.emit('download-done', download);
          download.done = true;
        });
    });
